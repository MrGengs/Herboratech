/**
 * chat-manager.js — HerboraTech Real-time Chat via Firestore
 * =========================================================
 * Usage:
  ...
  static renderBubble(msg) {
    ...
    if (msg.type === 'vital_share') {
      ...
      return `<div class="chat-bubble ${msg.isMine ? 'sent' : 'received'}" style="padding:0;overflow:hidden;max-width:85%;">
        <div style="background:rgba(14,124,70,0.1);border:1px solid rgba(14,124,70,0.3);border-radius:14px;padding:12px 14px;">
          <div style="font-size:0.7rem;font-weight:700;color:#0E7C46;margin-bottom:6px;">📊 DATA VITAL SIGN</div>
          ...
          <div style="font-size:0.6rem;color:#9E9EA4;margin-top:4px;text-align:right;">${timeStr}</div>
        </div>
      </div>`;
    }
    ...
    if (msg.type === 'prescription') {
      return `<div class="chat-bubble ${msg.isMine ? 'sent' : 'received'}" style="padding:0;overflow:hidden;max-width:85%;">
        <div style="background:rgba(47,125,86,0.1);border:1px solid rgba(47,125,86,0.3);border-radius:14px;padding:12px 14px;cursor:pointer;">
          <div style="font-size:0.7rem;font-weight:700;color:#2F7D56;margin-bottom:4px;">📋 RESEP DIGITAL</div>
