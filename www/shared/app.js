...
// ── SVG Sparkline ──
function sparkline(container, data, color = '#2F6F4F', h = 36) {
...
function createGauge(container, value, max = 100, color = '#1D4F38') {
  if (!container) return;
  const r = 42;
...
// ── Bar Chart (simple) ──
function miniBarChart(container, data, labels, color = '#2F6F4F') {
  if (!container) return;
  const max = Math.max(...data);
...
function showToast(msg, type = 'success') {
  const colors = { success: '#1D4F38', error: '#B23A34', warning: '#8F6018', info: '#2C6E9B' };
